...
export class RealKekaAdapter implements ATSAdapter {
  ...
  async getJobs(): Promise<KekaJob[]> {
    ...
    return rawJobs.map((job: any) => ({
      ...
      status: (job.status === 2 || job.status === 1 || job.status === 0 || job.status === undefined) && job.status !== 4 && job.status !== 5 ? "active" : "closed",
      ...
      createdAt: job.createdOn ? new Date(parseFloat(job.createdOn) * 1000) : undefined,
      updatedAt: job.publishedOn ? new Date(parseFloat(job.publishedOn) * 1000) : undefined,
      external_id: job.id,
      source_system: "Keka",
      sync_status: "synced",
