...
import { Clock, ShieldAlert, ChevronLeft, ChevronRight, LifeBuoy } from "lucide-react";
...
export default function ExamInterface({
  ...
  const [supportSubject, setSupportSubject] = React.useState("Technical Issue");
  ...
  const handleSupportSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (resp.ok) {
        toast.success("Support ticket submitted! Our team will assist you shortly.");
        setIsSupportOpen(false);
        setSupportMessage("");
      } else {
        const errData = await resp.json();
        toast.error(errData.error || "Failed to submit support ticket.");
      }
    } catch (err) {
      console.error(err);
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col justify-between select-none">
        ...
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="flex items-center gap-1.5 bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-700 text-[10px] font-bold px-3 py-1.5 rounded-lg transition-colors cursor-pointer"
            >
      ...
      {isSupportOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6 border border-slate-200 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-start gap-4">
              <div className="h-10 w-10 bg-indigo-50 rounded-full border border-indigo-200 flex items-center justify-center text-indigo-600 shrink-0">
                <LifeBuoy className="h-5 w-5" />
              </div>
              <div className="space-y-1 flex-1">
                <h3 className="text-base font-bold text-slate-900">Need Technical Support?</h3>
                <p className="text-xs text-slate-500 leading-relaxed">
                  Describe the technical issue you are experiencing, and we will notify the recruiter.
                </p>
              </div>
            </div>
            ...
            <form onSubmit={handleSupportSubmit} className="mt-4 space-y-4">
              <div className="space-y-1">
                <label className="block text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Issue Category</label>
                <select
                  value={supportSubject}
                  onChange={(e) => setSupportSubject(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 text-slate-800 rounded px-2.5 py-1.5 font-sans font-bold text-xs outline-none focus:border-indigo-500 transition-colors"
                >
                  <option value="Technical Issue">Technical / Loader Issue</option>
                  <option value="Question Display">Question Display Issue</option>
                  <option value="Proctoring Warning">False Proctoring Warning</option>
                  <option value="Other Issue">Other general issue</option>
                </select>
              </div>
              
              <div className="space-y-1">
                <label className="block text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Describe the Issue</label>
                <textarea
                  rows={4}
                  value={supportMessage}
                  onChange={(e) => setSupportMessage(e.target.value)}
                  placeholder="Describe what went wrong in detail..."
                  className="w-full bg-slate-50 border border-slate-200 text-slate-800 rounded p-2.5 font-sans text-xs outline-none focus:border-indigo-500 transition-colors resize-none leading-relaxed"
                  required
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-3 border-t border-slate-100 pt-4">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-lg transition-colors cursor-pointer shadow-md disabled:opacity-50"
                >
                  {isSubmittingSupport ? "Sending Request..." : "Submit Support Request"}
                </button>
              </div>
            </form>
          </div>
